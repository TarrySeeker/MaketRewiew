...
export function ServiceCard({ service }: ServiceCardProps) {
    ...
    return (
        <Card className="flex flex-col h-full hover:border-brand-silver/50 transition-colors group">
            <div className="aspect-video relative overflow-hidden rounded-t-lg bg-brand-dark">
                <img
                ...
                />
                <div className="absolute top-2 right-2 bg-background/80 backdrop-blur text-foreground px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border border-brand-gray">
                    {service.category}
                </div>
            </div>
            ...
            <CardHeader>
                <div className="flex justify-between items-start gap-2">
                    <CardTitle className="text-xl group-hover:text-brand-silver transition-colors">{service.title}</CardTitle>
                </div>
                ...
                <ul className="space-y-2 text-sm text-brand-silver">
                    {service.features && service.features.map((feature, i) => (
                        <li key={i} className="flex items-center">
                            <CheckCircle2 className="h-4 w-4 mr-2 text-brand-silver" /> {feature}
                        </li>
                    ))}
                </ul>
            </CardContent>

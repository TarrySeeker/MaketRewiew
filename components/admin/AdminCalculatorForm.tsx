...
export function AdminCalculatorForm() {
    ...
    return (
        <div className="space-y-8 bg-brand-dark p-8 border border-brand-gray max-w-2xl mx-auto rounded-lg">
            <div className="flex items-center justify-between border-b border-brand-gray pb-6">
                <div className="space-y-1">
                    <h2 className="text-2xl font-heading font-bold text-foreground uppercase">Настройки Калькулятора</h2>
                    <p className="text-brand-silver text-sm">Управление базовыми ставками и коэффициентами.</p>
                </div>
                <Link href="/" className="text-brand-silver hover:text-foreground transition-colors">
            ...
            {Object.entries(groupedSettings).map(([category, items]) => (
                <div key={category} className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <h3 className="text-xs font-bold text-foreground uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                    ...
                    <div className="grid gap-4 bg-background p-4 rounded-md border border-brand-gray">
                        {items.map((setting) => (
                            <div key={setting.id} className="grid grid-cols-2 items-center gap-4">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full bg-background border border-brand-gray px-4 py-3 text-foreground focus:border-foreground focus:outline-none focus:ring-1 focus:ring-foreground transition-all text-right font-mono"
                                    />
                                    {category === 'weld_type' && (
            ...
            <div className="pt-4">
                <Button
                    onClick={handleSave}
                    disabled={saving}
                    className="w-full font-bold uppercase tracking-widest h-14 rounded-none transition-all hover:scale-[1.01] active:scale-[0.99]"
                >
